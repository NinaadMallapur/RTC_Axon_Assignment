...
static void cli__task_list_print(sl_string_s output_string,
  ...
  cli_output(unused_cli_param, sl_string__c_str(output_string));
#else
  cli_output(unused_cli_param, "Unable to provide you the task information "
                               "along with their CPU and stack usage.\n");
  cli_output(
      unused_cli_param,
      "configUSE_TRACE_FACILITY macro at FreeRTOSConfig.h must be non-zero\n");
#endif
}
